...
for i in range(0, cnt):
    pretot = traindata[i][5] * wtrain[4]
    for j in range(0, 4):
        pretot += wtrain[j] * traindata[i][j]
    if sign(pretot) != traindata[i][4]:
        preerror += 1
tmppreerror = preerror

arr = np.zeros(1126)
for time1 in range(1126):
    wtrain = np.zeros(5)
    besttrain = np.zeros(5)
    preerror = tmppreerror
    time = 1
    while True:
        ...
        if sign(total) != traindata[index][4]:
            time += 1
            for k in range(0, 4):
                wtrain[k] += traindata[index][4] * traindata[index][k]
            wtrain[4] += traindata[index][5] * traindata[index][4]
            error = 0
            for k in range(0, cnt):
                t = traindata[k][5] * wtrain[4]
                for l in range(0, 4):
                    t += wtrain[l] * traindata[k][l]
                if sign(t) != traindata[k][4]:
                    error += 1
            if error < preerror:
                #print(error)
                preerror = error
                for l in range(0, 5):
                    besttrain[l] = wtrain[l]
        if time == 100:
            break
    error1 = 0
    for i in range(0, cnt2):
        total1 = testdata[i][5] * besttrain[4]
        for j in range(0, 4):
            total1 += besttrain[j] * testdata[i][j]
